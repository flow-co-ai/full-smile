...
GH_REPO = E('GITHUB_REPO', 'flow-co-ai/fullsmile-dashboard')
...
DAYS_AHEAD = int(E('FS_DAYS_AHEAD', '30'))
...
def opendental(a, b, ahead):
    ...
    out['providers'] = od_query("SELECT ProvNum AS prov, Abbr AS abbr, FName AS f, LName AS l FROM provider")
    ...
    out['newPatients'] = od_query(f"""SELECT p.PatNum AS pat, p.DateFirstVisit AS fv, p.WirelessPhone AS m, p.HmPhone AS h,
        p.WkPhone AS w, p.Email AS e FROM patient p WHERE p.DateFirstVisit BETWEEN '{a}' AND '{b}' AND p.PatStatus <> 4""")
    ...
    out['ref'], out['npColl'], out['npProd'], out['npPlan'] = [], [], [], []
...
def build():
    ...
    for r in od['newPatients']:
        ...
        if rec and rec['lead']:
            ...
            quality['none'] += 1
        key = (monday(fv), ch, by)
        x = np_weeks.setdefault(key, [0, 0.0, 0.0, 0.0])
        x[0] += 1; x[1] += coll.get(pat, 0); x[2] += prod.get(pat, 0); x[3] += plan.get(pat, 0)
    new_patients = [[w, ch, by, v[0], round(v[1], 2), round(v[2], 2), round(v[3], 2)] for (w, ch, by), v in sorted(np_weeks.items())]
    ...
    return {
        'version': 1, 'generatedAt': dt.datetime.utcnow().replace(microsecond=0).isoformat() + 'Z', 'asOf': b, 'window': [a, b],
        'practice': {'daily': daily, 'prodCat': prod_cat, 'prodProv': prod_prov, 'providers': providers, 'snapshot': snapshot},
...
def gh(method, path, body=None):
    return http(method, f'https://api.github.com/repos/{GH_REPO}/{path}',
...
if __name__ == '__main__':
    ...
    doc = build()
    q = doc['quality']
    print(f"Built: {len(doc['practice']['daily'])} days, {sum(r[3] for r in doc['newPatients'])} new patients "
